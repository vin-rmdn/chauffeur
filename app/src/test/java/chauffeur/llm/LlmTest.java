...
public class LlmTest {
  ...
  @InjectMocks
  private LargeLanguageModel classInTest;

  @BeforeEach
  void setUp() {
    MockitoAnnotations.openMocks(this);
    classInTest = new LargeLanguageModel(mockModel);
  }
  ...
  void TestLlm_Successful() {
    ...
    LargeLanguageModelResponse llmResponse = classInTest.converse("Hello there!", null);
  ...
  void TestLlm_Successful_WithChatHistory() {
    ...
    chatHistory.add(
        new Pair<String, String>("assistant", "You should. Malls are center for shopping places."));

    String mockResponse =
        "Yes, I remember. This is a mocked response of an LLM so test should work properly";
    when(mockModel.complete(any(InferenceParameters.class))).thenReturn(mockResponse);

    LargeLanguageModelResponse llmResponse =
        classInTest.converse("Remember what I was asking previously?", chatHistory);
    assertEquals(llmResponse.response, mockResponse);

    List<Pair<String, String>> expectedChatHistory = new ArrayList<>();
    expectedChatHistory.add(new Pair<>("user", "Should I go to the malls?"));
    expectedChatHistory
        .add(new Pair<>("assistant", "You should. Malls are center for shopping places."));
